package FlightTracker.main.java.Application.Service;
...
public class FlightService {
     ...
     * This is used for when a mock flightDAO that exhibits mock behavior is used in
     * the test cases.
     ...
     *
     * This method should also return the added flight. A distinction should be made
     ...
     * because it is not yet a database record. When this method is used, it should
     * return the full persisted flight,
     * which will contain the flight's id. This way, any part of the application
     * that uses this method has
     * all information about the new flight, because knowing the new flight's ID is
     ...
     *         inform our provide the front-end client with information about the
     *         added Flight.
     */
     ...
     * You should first check that the flight ID already exists. To do this, you
     * could use an if statement that checks
     * if flightDAO.getFlightById returns null for the flight's ID, as this would
     * indicate that the flight id does not
     * exist.
     *
     * @param flight_id the ID of the flight to be modified.
     * @param flight    an object containing all data that should replace the values
     *                  contained by the existing flight_id.
     *                  the flight object does not contain a flight ID.
     * @return the newly updated flight if the update operation was successful.
     *         Return null if the update operation was
     *         unsuccessful. We do this to inform our application about
     *         successful/unsuccessful operations. (eg, the
     *         user should have some insight if they attempted to edit a nonexistent
     *         flight.)
     */
    public Flight updateFlight(int flight_id, Flight flight) {
        System.out.println("flight_id: " + flight_id + " // flight: " + flight);
        Flight existingFlight = flightDAO.getFlightById(flight_id);
        System.out.println("existingFlight: " + existingFlight);
        if (existingFlight != null) {
            String departureCity = flight.getDeparture_city();
            String arrivalCity = flight.getArrival_city();
            Flight flightObject = new Flight(flight_id, departureCity, arrivalCity);
            // Flight updatedFlight = flightDAO.updateFlight(flight_id, flightObject);
            System.out.println("flightObject: " + flightObject);
            return flightObject;
        } else {
            return null;
        }
    ...
    /**
     * TODO: Use the FlightDAO to retrieve a List containing all flights departing
     * from a certain city and arriving at
     * some other city. You could use the flightDAO.getAllFlightsFromCityToCity
     * method.
     *
     * @param departure_city the departing city of the flight.
     * @param arrival_city   the arriving city of the flight.
     * @return all flights departing from departure_city and arriving at
     *         arrival_city.
     */
    public List<Flight> getAllFlightsFromCityToCity(String departure_city, String arrival_city) {
        List<Flight> p2pFlights = flightDAO.getAllFlightsFromCityToCity(departure_city, arrival_city);
